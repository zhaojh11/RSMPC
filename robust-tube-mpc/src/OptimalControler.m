...
%             obj.x_max = max(Xc.V, [], 2)';
        ...
        function add_UC_bond_ineq_constraint(obj, UC_bond)
            ...
            min_v = UC_bond(1,2);
            max_v = UC_bond(1,1);
            % UC_vertex = [max_v,max_v;max_v,min_v;min_v,min_v;min_v,max_v];
            % Uc_ = Polyhedron(UC_vertex);
            % if Uc_.contains(zeros(2, 1)) % If Xadd contains the origin, the contraint can be expressed as C1*x<=1
            %     [G_add, ~, ngc_add] = convert_Poly2Mat(Uc_, Polyhedron());
        ...
        function [x_seq, u_seq,exitflag] = solve(obj,Vold)
            quadprog_solved = 0;
...
%                 C_ss_eq1(:, idx_xk_start:idx_xk_end) = obj.sys.A; %改动
        ...
        function [C_ineq1, C_ineq2] = construct_ineq_constraint(obj, X_v, Uc)
            % compute C_ineq
            C_ineq1 = [];
            C_ineq2 = [];
            for i=2:obj.N
...
%                 C_ineq2 = [C_ineq2 ; ones(nc_total, 1)];
            end
...
%             X_K = X_v(1);
%             [F, G, nc] = convert_Poly2Mat(X_K, Uc);
%             
%             F_block = [];
%             G_block = [];
%             for itr = 1:obj.N
